...
Configure an MCP client to launch `dotnet` with arguments `exec` and the absolute path to `src/AutoGIS.Civil3D.Mcp/bin/Release/net8.0/AutoGIS.Civil3D.Mcp.dll`. Set `AUTOGIS_MCP_BUNDLE_ROOT` in the child environment to an absolute, owner-controlled directory containing handoff ZIP files. The server has no default root. Keep that directory stable while validation runs; path checks do not protect against another process replacing entries concurrently.
