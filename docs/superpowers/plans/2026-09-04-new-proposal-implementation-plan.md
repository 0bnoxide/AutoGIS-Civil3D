...
Follow [the coordination lifecycle](../../collaboration.md) for each slice. Claim the branch, worktree, and paths before writing; keep `AGENT_SESSION_ID` set for real writes. Track execution on a Phase 4 delivery issue with child work items corresponding to these tasks. Do not allocate an ADR number in this plan; ADR-0008 records the authorized installed-SDK sourcing extension.
...
- [ ] Exercise the [approved receipt-publication lifecycle](../specs/2026-09-04-civil-production-accelerator-design.md#execution-lifecycle) **before Task 4**, under the [owner decision on issue #101](https://github.com/0bnoxide/AutoGIS-Civil3D/issues/101#issuecomment-5744045745). Retain the post-promotion unwritable-receipt counterexample and inject failures at receipt preparation, promotion, and post-publication presentation. Assert the design's final-root, retained-evidence, cleanup-ownership, and user-visible outcomes. Filesystem evidence does not replace the required native API and handle-release observations.
...
- [ ] Write failing `ProposalExecutorTests` using an OS temporary directory and the Task 1 fixture remapped to that directory. Start with approval refusal and existing-target refusal; assert byte-for-byte unchanged existing contents and no host mutation calls. Then inject failure at every action boundary, close, verification, success-receipt preparation/flush, and promotion. Assert no owned final root, preservation of foreign targets, retained failure receipt, exact failed action, and visible cleanup failure. Separately inject presentation failure after publication: assert successful creation, unchanged published receipt, preserved final root, and a visible presentation warning with both paths.
...
- [ ] Reserve a unique sibling staging directory, record ownership, and execute only its planned actions, deferring the `ReceiptAfterVerification` support action until verification is complete. Never adopt an existing staging directory. Track created handles and ensure close is attempted on success and failure. Verify using the host's independent readback, close verification handles, and compare its observations with the plan. Prepare, flush, and close the complete success receipt in staging, then promote the root and receipt together without overwrite under the design's approved lifecycle.
