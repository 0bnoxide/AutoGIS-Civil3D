...
**Tech stack:** .NET 8, C# 12, System.Text.Json, existing xUnit packages, Windows Forms for the small native wizard, and the retained Civil 3D 2025 reference-assembly approach. Native SDK calls and Sheet Set integration must be established by Task 3 probes before they become implementation commitments.
...
- Retain the [adapter targeting and reference sourcing decisions](../specs/2026-09-04-phase-4-adapter-foundation-design.md#reference-assembly-sourcing) as qualified by ADR-0006. A different release or sourcing policy needs its own owner decision.
...
- [ ] Add `HostCompatibilityTests` for the retained supported series and mismatched series. Add `PreviewTests` asserting that every action, final target, relevant input, and manual limitation appears once and in plan order. Keep the renderer free of Autodesk types so ordinary CI can exercise it.
...
- [ ] Ask the design's open owner inputs one at a time at the point of use. The folder/template/profile and sheet naming inputs gate real creation; title-block/page-setup and placeholder inputs gate the sheet writer; release and Sheet Set Manager mode gate the relevant host probe. Synthetic inputs unblock experiments, not claims about company compatibility. Link to the design's input list rather than copying it into a new tracker.
...
**Consumes:** The reviewed implementation, answered owner inputs, and real approved templates on the retained supported host.
