...
1. Open the ZIP; reconcile central metadata with local headers, descriptors, ZIP64 records, and physical entry spans; then enforce exact names — which jointly entail exactly the two required entries — regular-file status, regular-file status, compression methods, actual sizes, CRCs, and ratio limits.
