...
These are decision-states: each records an owner decision, never live work state. Whether authorized work is underway, blocked, or under review is represented by GitHub issues and pull requests, not by this file.

Only an explicit owner decision may authorize, advance, reorder, or reopen a phase. Every such decision is appended to the gate-change log below as one row carrying PR and ADR references only — evidence prose, counts, and delivery detail belong to the referenced records. A change to phase ordering or architecture also requires an ADR.
...
Phases 1 and 2 ran ahead of Phase 0 by explicit owner decision (gate-change log, 2026-08-02; [ADR-0003](adr/0003-contract-slice-precedes-phase-0.md)) and were accepted together on 2026-08-04. The capability sequence above is otherwise unchanged; the deviation is recorded, not rewritten.
...
Append-only: one row per owner decision, references only; corrections are new rows, never edits. Rows predating the 2026-08-05 log discipline (issue #15) retain their original evidence prose unedited.
